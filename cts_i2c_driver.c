		...
		const struct i2c_device_id *id)
{
	...
	ret = devm_request_threaded_irq(&cts_data->pdata->i2c_client->dev,gpio_to_irq(TOUCH_GPIO_INT),NULL,cts_plat_irq_handler,IRQF_ONESHOT|IRQF_TRIGGER_FALLING,"touch irq",cts_data);	
	if(ret)
	{		
...
static DRIVER_ATTR_RO(i2c_xfer_size, S_IRUGO, i2c_xfer_size_show, NULL);
